from RL_Snake.Board import Board
from RL_Snake.Snake import Snake
from RL_Snake.GUI import App
import random
...
class Game:
    ...
    def __init__(self,board_size,agent,start_board_snake=None,cur_reward=0):
        ...
        if start_board_snake is None:
            ...
            self.board = start_board_snake[0]  # 0th is the board#
            self.snake = start_board_snake[1]  # 1st is the snake
    ...
    def one_time_step(self):
        ...
        if out == 10:          # if out == -10 , hit the wall and game over
            self.cur_reward += 1
            i,j = self.new_food_location()
            self.board.set_cell(i,j,-20)
            self.board.set_food((i,j))
    ...
    def receive_action(self):
        ...
        """
        action = self.agent.take_action().item()   # return action: int
        if action < len(ACTIONS):                  # ACTION:[0,1,2,3]
